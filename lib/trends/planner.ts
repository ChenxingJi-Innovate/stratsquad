...
import type { TrendQueryPlan, TrendQuery } from './types'
...
1. google-trends - 全球关键词搜索热度，可选 region (US/JP/KR/ID/VN/PH/TH/MY/SG/TW/HK/GB/FR/DE/CN/WW)、timeframe (默认 "today 12-m")。返回相对值 0-100。
2. steam - PC 游戏当前在玩量。可传 gameTitles 查指定游戏，或不传查 Top 10。
3. twitch - 海外直播品类热度。需要 gameTitles 或不传查 Top 10。Twitch 在中国大陆无用，不要为中国市场问题选这个。
4. reddit - 海外社区情绪。需要 subreddits (如 ["MobileGaming","gachagaming","GenshinImpact"]) 和/或 keywords。
5. youtube - 视频热度。需要 keywords，可选 region (默认 US)。
6. appstore - iOS App Store 游戏榜。需要 region (国家二字代码如 us/cn/jp/kr/id/vn/ph/sg/th)。
7. huya - 中国虎牙直播。需要 category (取值之一: ${KNOWN_HUYA_CATEGORIES.join(', ')})。海外问题不要选。
8. douyu - 中国斗鱼直播。需要 category (取值之一: ${KNOWN_DOUYU_CATEGORIES.join(', ')})。海外问题不要选。
9. bilibili - 哔哩哔哩。两种用法之一: 传 category (取值之一: ${KNOWN_BILIBILI_CATEGORIES.join(', ')}) 查直播; 或传 keywords 查视频。

规则:
- 选 4-7 个源，不要全选。把"区域适配"放在第一位。
