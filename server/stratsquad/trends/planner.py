...
SOURCE_DESC: dict[TrendSource, str] = {
    ...
    "huya": f"7. huya - 中国虎牙直播。需要 category (取值之一: {', '.join(KNOWN_HUYA)})。海外问题不要选。",
    "douyu": f"8. douyu - 中国斗鱼直播。需要 category (取值之一: {', '.join(KNOWN_DOUYU)})。海外问题不要选。",
    "bilibili": f"9. bilibili - 哔哩哔哩。两种用法之一: 传 category (取值之一: {', '.join(KNOWN_BILIBILI)}) 查直播; 或传 keywords 查视频。",
}
